...
class hard:
    ...
    def __init__(self, crosshair_type):
        ...
        while (starting_scene):
            screen.blit(con.medium_start, (0, 0))
        ...
        while (running):
            ...
            """
            if con.Target_timer >= con.Target_maxtimer_hard or aim.is_collision():
                self.t = random.randint(0,1)
                if self.t :
                    target = Target(con.Target1)
                else:
                ...
                if con.add_point:
                    ...
                    con.Count += 1
                    con.points += 5
                    if con.Target_maxtimer_hard > con.Target_mintimer_hard:
                        con.Target_maxtimer_hard -= .5
                    elif con.Target_maxtimer_hard > 10:
                        con.Target_maxtimer_hard -= .1
                else:
                    con.miss_count += 1
                    con.points -= 3
                aim.list = []
            ...
            ###########ENding Game#################################
            if con.points <= 0 or int(con.target_no_variable) >= int(con.no_of_targets):
                running = False
                end_running = True
                while end_running:
